...
```bash
# Windows图标
copy G:\BS\logo.png G:\BS\desktop-pyqt\logo.png

# macOS图标
copy G:\BS\logo.icns G:\BS\desktop-pyqt\logo.icns
```
...
# 打包
pyinstaller --name "AI旅行规划助手" --windowed --onedir --icon logo.icns --add-data "logo.icns:." --hidden-import webview --hidden-import webview.platforms.cocoa --noconfirm main_mac_webview.py
...
```bash
# 使用Rosetta运行（如需要）
arch -x86_64 python3 -m PyInstaller --name "AI旅行规划助手" --windowed --onedir --icon logo.icns --add-data "logo.icns:." --hidden-import webview --hidden-import webview.platforms.cocoa --noconfirm main_mac_webview.py
...
```bash
cd G:\BS\desktop-pyqt && rmdir /s /q build dist 2>nul & del *.spec 2>nul & pyinstaller --name "AI旅行规划助手" --windowed --onedir --icon logo.png --add-data "logo.png;." --hidden-import PyQt6 --hidden-import PyQt6.sip --hidden-import PyQt6.QtWebEngineWidgets --hidden-import PyQt6.QtWebEngineCore --hidden-import PyQt6.QtWebChannel --noconfirm main_webview.py
```

### macOS一行命令

```bash
cd desktop-pyqt && rm -rf build dist *.spec && pyinstaller --name "AI旅行规划助手" --windowed --onedir --icon logo.icns --add-data "logo.icns:." --hidden-import webview --hidden-import webview.platforms.cocoa --noconfirm main_mac_webview.py && xattr -cr dist/AI旅行规划助手.app
```

### Linux一行命令

```bash
cd desktop-pyqt && rm -rf build dist *.spec && pyinstaller --name "AI旅行规划助手" --windowed --onedir --icon logo.png --add-data "logo.png:." --hidden-import PyQt6 --hidden-import PyQt6.sip --hidden-import PyQt6.QtWebEngineWidgets --hidden-import PyQt6.QtWebEngineCore --hidden-import PyQt6.QtWebChannel --noconfirm main.py
```
