...
| 后端端口 | `3000`（nginx 反向代理到 `http://localhost:3000`） |
...
# 智谱AI（OpenAI兼容接口，也可填小米 MiMo 等，详见 server/.env.example）
AI_API_KEY="你的AI服务API Key"
...
server {
    listen 80;
    server_name example.com;  # 替换为你的域名（如 wanghaiyan.cn）或服务器IP
    ...
    # 后端API（端口需与 PM2 中 travel-server 实际监听端口一致，默认 3000）
    location /api {
...
首次部署完成后运行 `node server/create-admin.mjs` 自行创建管理员：

```bash
cd /opt/travel-planner/server
ADMIN_EMAIL="admin@example.com" ADMIN_PASSWORD="你的强密码" node create-admin.mjs
```

说明：

- 未设置 `ADMIN_EMAIL` 时脚本会使用默认邮箱（建议始终显式指定自己的邮箱）
- `ADMIN_PASSWORD` 为必填，未设置时脚本会直接报错退出
- 创建出的账号角色为 `admin`，请使用自己设置的高强度密码并妥善保管
